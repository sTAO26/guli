...
public class OrderServiceImpl extends ServiceImpl<OrderDao, OrderEntity> implements OrderService {
    ...
    public void createSeckillOrder(SeckillOrderTo order) {
        ...
        // 保存SKU信息！！
        R r2 = productFeignService.getSkuInfoBySkuId(order.getSkuId());
//        SkuInfoEntity skuInfo = r2.getData(new TypeReference<SkuInfoEntity>() {});
        SkuInfoEntity skuInfo = r2.getData("skuInfo",new TypeReference<SkuInfoEntity>() {});
        ...
        System.out.println("lalala:"+orderItem);
        // 保存订单项数据
        orderItemService.save(orderItem);
    }

    @Override
    public List<BuyTimeTO> getBuytime(Long memberid) {

//        HashMap<Long,Integer> ret  = new HashMap<>();
        List<BuyTimeTO> ret  = new ArrayList<>();

        //获取订单编号列表
        List<OrderEntity> orderEntities = baseMapper.selectList(new QueryWrapper<OrderEntity>()
                .eq("member_id", memberid));
        if(orderEntities.isEmpty()) return ret;
        List<String> OrderSns = orderEntities.stream().map(order -> order.getOrderSn()).collect(Collectors.toList());
        //获取购买的商品列表
        Map<String, List<OrderItemEntity>> itemMap = orderItemService.list(new QueryWrapper<OrderItemEntity>()
                        .in("order_sn", OrderSns)).stream().collect(Collectors.groupingBy(OrderItemEntity::getOrderSn));
        for(String ordersn : itemMap.keySet()){
            List<OrderItemEntity> itemlist = itemMap.get(ordersn);
            for(OrderItemEntity item : itemlist){
//                if(!ret.containsKey(item.getSkuId())){
//                    ret.put(item.getSkuId(),item.getSkuQuantity());
//                }
//                else{
//                    ret.put(item.getSkuId(),ret.get(item.getSkuId()) + item.getSkuQuantity());
//                }
                Long skuid = item.getSkuId();
                Integer time = item.getSkuQuantity();
                ret.add(new BuyTimeTO(skuid,time));
