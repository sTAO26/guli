...
public class MemberServiceImpl extends ServiceImpl<MemberDao, MemberEntity> implements MemberService {
     ...
     */
    @Override
    public MemberEntity login(SocialUserTO user) throws Exception {
        // 1.判断当前用户是否已经在本系统注册
        String uid = user.getUid();
        MemberEntity _entity = baseMapper.selectOne(new QueryWrapper<MemberEntity>().eq("social_uid", user.getUid()));
        if (_entity != null) {
            // 2.已注册，更新令牌、过期时间后返回即可
            ...
            try {
                ...
                HttpResponse response = HttpUtils.doGet("https://gitee.com", "/api/v5/user", new HashMap<String, String>(), queryMap);
                if (response.getStatusLine().getStatusCode() == 200) {
                    ...
                    String profileImageUrl = jsonObject.getString("avatar_url");//头像
                    // 封装注册信息
